...
- `聲音工作台`：依目前配方播放 FARHP-Y 的 PH16 代表性合成，提供中性／男聲／女聲、固定自動示範、可重播隨機示範與 WAV 匯出；
...
所有網站相依項目皆使用相對路徑。正式網站使用 `https://axioglyph.evemisslab.com/`；公開部署不直接暴露本目錄全部內容，根目錄的 `site/build.py` 只會把瀏覽器需要的檔案打包到 `site/dist/`，再由 assets-only Worker 發布。
...
聲音工作台的男／女／中性選項是人工合成聲線預設，不是生理分類。PH16 只有可視簽名，因此頁面生成的是固定、可重現的代表向量，不是唯一發音；沒有完整反演資料的 FARHP-G 會拒絕播放。
